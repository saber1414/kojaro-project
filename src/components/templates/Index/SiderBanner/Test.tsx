...
const SliderBanner = () => {
  ...
  // وقتی activeIndex تغییر می‌کند، تصویر پس‌زمینه عوض می‌شود
  useEffect(() => {
    // تصویر پس‌زمینه با activeIndex تغییر می‌کند
  }, [activeIndex]);
  ...
  return (
    <section className='w-full h-165 relative overflow-hidden'>
      {/* تصویر پس‌زمینه */}
      <div
        className='absolute inset-0 bg-cover bg-center bg-no-repeat blur-lg scale-105 transition-all duration-800 ease-in-out'
              ...
              <SwiperSlide key={slide.id}>
                <div className="flex items-center gap-x-6 px-4 mx-auto mt-35">
                  <Link href={slide.link}>
                    <img src={slide.image} alt={slide.title} className='rounded-sm w-60 h-40 object-cover' />
                  </Link>
                  <div className="flex flex-col gap-y-4">
                    <Link href={slide.link}>
                      <span className='text-3xl text-white font-IRANYekan-Bold leading-normal'>
                        {slide.title}
                      </span>
                    </Link>
                    <Link href={slide.link} className='flex items-center justify-center gap-x-4 font-IRANYekan-Bold bg-blueMenu text-white h-9.5 w-41.25 rounded-lg hover:bg-blue-700 transition-colors'>
                      <span className='text-[13px]'>مشاهده ادامه مطلب</span>
                      <svg width="16" height="16" viewBox="0 0 256 256" className="fill-white">
            ...
            {slides.map((slide, index) => {
              ...
              return (
                <div 
                  ...
                  className={`flex-1 flex flex-col gap-y-3 cursor-pointer group transition-all ${
                    isActive ? 'opacity-100' : 'opacity-60 hover:opacity-80'
                  }`}
                >
                  <span className={`text-[17px] font-IRANYekan-Bold transition-colors truncate ${
