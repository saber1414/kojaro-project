...
const ArticleBox = () => {
  ...
  const [progress, setProgress] = useState<number>(0);
    ...
    const scrollToCommentBox = () => {
        ...
        if (commentBox) {
            const yOffset = -80; // برای هدر sticky
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (scrolledFromArticle <= 0) {
        progressPercent = 0
      } else if (scrolledFromArticle >= articleHeight - window.innerHeight) {
        progressPercent = 100
      } else {
        progressPercent = (scrolledFromArticle / (articleHeight - window.innerHeight)) * 100;
      };
  ...
  return (
    <div className='w-full pt-8 pb-2 px-8 container'>
      <div className="flex gap-x-4">
        ...
        <div ref={articleContentRef} className='basis-[70%]'>
          <h1 className='font-IRANYekan-Bold text-2xl'>شادترین شهر جهان در سال ۲۰۲۶ معرفی شد؛ ۲۰ شهری که شما را غافلگیر می‌کند</h1>
